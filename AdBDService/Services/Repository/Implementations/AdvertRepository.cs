...
namespace AdDBService.Services.Repository.Implementations
{
    public class AdvertRepository : IAdvertRepository<Advert>
    {
        ...
        public async Task<Guid> Add(Advert entity, IFormFile? uploadedFile = null)
        {
            ...
            if (db.Adverts.Where(adv => adv.UserId == entity.UserId).Count() >= DisplayConst.MaxUserAdvertCount)
            {
                Console.WriteLine($"Пользователь '{entity.UserId}' " +
                                  $"достиг лимита количества обьявлений ({DisplayConst.MaxUserAdvertCount}).");
                return Guid.Empty;
            }
            ...
            try
            {
                ...
                if(uploadedFile != null && uploadedFile.Length <= DisplayConst.MaxAdvertsPhotoWeight)
                {
                    string pathForPicture = $"{DisplayConst.PathForAdvertsPhoto}{entity.Number}.JPG";
